...
class LaTeXBlockRenderer(BaseRenderer):
    ...
    def tex_escape(self, text): # https://stackoverflow.com/a/25875504
        """
        ...
        regex = re.compile('|'.join(re.escape(str(key)) for key in sorted(conv.keys(), key = lambda item: - len(item))), re.MULTILINE)
    ...
    def render_raw_text(self, token, escape=True):
        return self.tex_escape(token.content) if escape else token.content
